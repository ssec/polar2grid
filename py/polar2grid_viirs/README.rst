...
The `polar2grid.viirs` package contains objects and tools for extracting swath data from VIIRS SDR files.
These files are typically HDF5 files. Data files are typically read via the `Frontend`
object (`polar2grid.viirs.Frontend`). The frontend can also be accessed via the
command line by calling `python -m polar2grid.viirs -h`.
...
The python package itself can be installed via pip::
    ...
    pip install -i http://larch.ssec.wisc.edu/simple polar2grid.viirs

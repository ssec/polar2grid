...
GLUE_NAME = "viirs2ninjo"
LOG_FN = os.environ.get("VIIRS2NINJO_LOG", None) # None interpreted in main
...
def process_data_sets(nav_set_uid, filepaths,
    ...
    for grid_name,grid_dict in remapped_jobs.items():
        for (band_kind, band_id),band_dict in grid_dict.items():
            log.info("Running NinJo tiff backend for %s %s band grid %s" % (band_kind, band_id, grid_name))
            try:
                ...
                log.error("Error in the NinJo backend for %s%s in grid %s" % (band_kind, band_id, grid_name))
...
def main(argv = sys.argv[1:]):
    ...
    into a NinJo compatible tiff image for the DWD.
    """

...
hdf4 (.hdf) files and create a properly scaled geotiff file.
...
GLUE_NAME = "modis2gtiff"
LOG_FN = os.environ.get("MODIS2GTIFF_LOG", None) # None interpreted in main
...
def process_data_sets(nav_set_uid, filepaths,
    ...
    if len(remapped_jobs) == 0:
        log.warning("Geotiff backend failed for all grids for bands %r" % (band_info.keys(),))

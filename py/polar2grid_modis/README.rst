...
Polar2Grid MODIS Package
------------------------

The `polar2grid.modis` package contains objects and tools for extracting swath data from MODIS SDR files.
These files are typically HDF4 files. Data files are typically read via the `Frontend`
object (`polar2grid.modis.Frontend`). The frontend can also be accessed via the
command line by calling `python -m polar2grid.modis -h`.
...
The python package itself can be installed via pip::
    ...
    pip install -i http://larch.ssec.wisc.edu/simple polar2grid.modis

...
Polar2Grid DR-RTV Package
-------------------------

The `polar2grid.drrtv` package contains objects and tools for extracting swath data from files processed through
the University of Wisconsin's Dual Regression Retrieval (DR-RTV) algorithm. These files are typically HDf5 files.
Data files are typically read via the `Frontend` object (`polar2grid.drrtv.Frontend`). The frontend can also be
accessed via the command line by calling `python -m polar2grid.drrtv -h`.
...
The python package itself can be installed via pip::
    ...
    pip install -i http://larch.ssec.wisc.edu/simple polar2grid.drrtv

...
Polar2Grid MIRS Package
-----------------------

The `polar2grid.mirs` package contains objects and tools for extracting swath data from files processed through the
Microwave Integrated Retrieval System (MIRS). These files are typically NetCDF4 files. Data files are
typically read via the `Frontend` object (`polar2grid.mirs.Frontend`). The frontend can also be accessed via the
command line by calling `python -m polar2grid.mirs -h`.
...
The python package itself can be installed via pip::
    ...
    pip install -i http://larch.ssec.wisc.edu/simple polar2grid.mirs

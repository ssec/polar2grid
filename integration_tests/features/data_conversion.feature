Feature: Testing data conversions
   ...
   # test data located in /data/users/kkolman/test_data/ or kathys/test_data
   Examples: ACSPO
       | source            | command                            | output             |
       | acspo/input/test1 | acspo gtiff --grid-coverage=0.0 -f | acspo/output/test1 |
       | acspo/input/test2 | acspo gtiff --grid-coverage=0.0 -f | acspo/output/test2 |

   Examples: AMSR2_L1B
       | source            | command                                                  | output             |
       | amsr2/input/test1 | amsr2_l1b awips  -g 211e -p  btemp_36.5h btemp_89.0av -f | amsr2/output/test1 |

   Examples: AVHRR
       | source            | command         | output             |
       | avhrr/input/test1 | avhrr gtiff -f  | avhrr/output/test1 |                                       

   Examples: MIRS
       | source           | command                                | output            |
       | mirs/input/test1 | mirs awips -p tpw --grid-coverage=0 -f | mirs/output/test1 |

   Examples: MODIS
       | source            | command                                                                                                                           | output             |
       | modis/input/test1 | crefl gtiff --true-color --false-color --fornav-D 10 --grid-configs ${POLAR2GRID_HOME}/grid_configs/grid_example.conf -g miami -f | modis/output/test1 |
   ...
   Examples: VIIRS
    ...
    Examples: VIIRS-L1B
      ...
      # tol |viirs_l1b_day/l1b  |viirs_l1b awips -g 211e 211w -p adaptive_dnb dynamic_dnb --night-fraction=0.4 -f |viirs_l1b_day/awips   |
       | viirs_l1b_night/input/test1 | viirs_l1b gtiff --grid-configs /data/users/kkolman/test_data/viirs_l1b_night/output/test1/my_grid.conf -g polar_europe -p adaptive_dnb dynamic_dnb histogram_dnb hncc_dnb -f  | viirs_l1b_night/output/test1  |    


    Examples: VIIRS_SDR
       | source                      | command                                                            | output                       |
       | viirs_sdr_day/input/test1/  | viirs_sdr gtiff --i-bands --m-bands -p adaptive_dnb dynamic_dnb -f | viirs_sdr_day/output/test1   |
       # tol |viirs_sdr_night/sdr|viirs_sdr awips -g 211e -p adaptive_dnb dynamic_dnb -f|viirs_sdr_night/output_awips|
       # tol no hist |viirs_sdr_day/sdr|viirs_sdr awips -g 211e 211w -f|viirs_sdr_day/output_awips|

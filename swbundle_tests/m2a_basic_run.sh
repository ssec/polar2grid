#!/usr/bin/env bash
### Run simple tests to verify modis2awips will run properly ###
...
run_test() {
    ...
    modis2awips.sh -g $1 -d $2
    if [ $? -ne 0 ]; then
        echo "ERROR: modis2awips.sh did not complete test $2 successfully"
...
# Make sure we can call modis2awips.sh before making any test directories
which modis2awips.sh || oops "modis2awips.sh is not in PATH"
